...
def afghanistan_dashboard(request):
    return HttpResponse("Afghanistan Dashboard")
...
def get_view_json_data(request, filename):
    ...
    if view in ["COUNTRY", "PROVINCE", "DISTRICT"]:
        file_path = os.path.join(settings.BASE_DIR, 'afghanistan/static/data/DISTRICT', f'{filename}.json')
    elif view in ["BASIN", "WATERSHED"]:
        file_path = os.path.join(settings.BASE_DIR, 'afghanistan/static/data/WATERSHED', f'{filename}.json')
    else:
...
def get_json_data(request, filename):
    file_path = os.path.join(settings.BASE_DIR, 'afghanistan/static/data', f'{filename}.json')
    # Check if the file exists
    if os.path.exists(file_path):
        # Open the file and load the data
        with open(file_path, 'r') as file:
